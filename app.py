    ...
    while time.time() - st.session_state['focus_start_time'] < st.session_state.session_minutes * 60:
        ...
        if results.multi_face_landmarks:
            for landmarks in results.multi_face_landmarks:
                ...
                yaw_diff = abs(left_dist - right_dist)
                head_turn_threshold = 0.05
                head_turned = yaw_diff > head_turn_threshold

                # Combined condition for focus
                if not head_turned and ear > 0.2:
                    status = "Focused"
